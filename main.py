...
def button(update, context):
    ...
    if a == "play":
        context.bot.delete_message(chat_id=update.callback_query.message.chat_id,
                                   message_id=c)
        c = context.bot.createChatInviteLink(chat_id="@LuckGambleChat", member_limit=1)
        ink = c['invite_link']
        keyboard = [[InlineKeyboardButton("💬 Group Chat", url=ink)],
                    [InlineKeyboardButton("◀️ Back", callback_data="main_menu")]]
        reply_markup = InlineKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True)
        context.bot.send_message(chat_id=update.effective_user.id,
                                 text="🎲 Challenge a friend!\n\nTo play with your friend, join our group - @LuckGambleChat.",
...
def mines(update: Update, context: CallbackContext) -> None:
    ...
    if update.message.chat.type != 'group' and update.message.chat.type != 'supergroup':
        update.message.reply_text("You can only play Mines in the LuckGamble group chat.")
        return

    if update.message.chat_id != group_id:
        update.message.reply_text("This game is only available in the LuckGamble group.")
        return
...
def bal(update, context):
    ...
    if update.message.chat.type != 'group' and update.message.chat.type != 'supergroup':
        update.message.reply_text("You can only run this command in the LuckGamble group")
        return
                    ...
                    if nw == 3:
                        ...
                        context.bot.send_message(chat_id=group_id,
                                                 text="Score\n\n{}: {}\n{}: {}\n\n{} wins and gets ${}!".format(p1n, nw,
                                                                                                                ...
                                                                                                                f"{am:.2f}"),
                                                 reply_to_message_id=update.message.message_id)
                        context.bot.send_message(chat_id="@LuckGambleWins", text="@{} just won ${} in 🎲Dice".format(p1n, f"{am:.2f}"))
                    else:
                        conn.execute(
                    ...
                    if nw == 3:
                        ...
                        context.bot.send_message(chat_id=group_id,
                                                 text="Score\n\n{}: {}\n{}: {}\n\n{} wins and gets ${}!".format(p1n,
                                                                                                                ...
                                                                                                                f"{am:.2f}"),
                                                 reply_to_message_id=update.message.message_id)
                        context.bot.send_message(chat_id="@LuckGambleWins", text="@{} just won ${} in 🎲Dice".format(p2n, f"{am:.2f}"))
                    else:
                        conn.execute(
                    ...
                    if balance > am:
                        ...
                        try:
                            ...
                            if "Successful" in msg:
                                ...
                                connection.close()
                                context.bot.send_message(chat_id=update.effective_user.id,
                                                         text="Your Withdrawl is sucessfull\n\nPlease feel free to leave a review here -> @LuckGambleVouch".format(balance),
                                                         reply_markup=reply_markup)
...
def help_command(update: Update, context: CallbackContext) -> None:
    ...
    help_text = (
        ...
        "🛠 **Need Help?**\n"
        "For assistance, reach out to **@Niggerlly**."
    )
...
def participate_rain_callback(update, context):
    ...
    if user.first_name and '@LuckGamble' not in user.first_name.lower():
        query.answer("⚠️ First name must contain '@LuckGamble'.", show_alert=True)
        return
...
def create_code(update, context):
    ...
    try:
        if len(context.args) != 5:
            update.message.reply_text("Usage:\n/createcode <code> <amount> <minwager> <usages> <trueorfalse to have LuckGamble on name>")
            return
        ...
        update.message.reply_text(
            ...
            f"Required LuckGamble: {is_diceable_required}",
            parse_mode=ParseMode.MARKDOWN
        )
    except Exception as e:
...
def claim_code(update, context):
    ...
    try:
        ...
        if is_diceable_required:
            if '@LuckGamble' not in (user.username or '').lower() and \
                    '@LuckGamble' not in (user.first_name or '').lower() and \
                    '@LuckGamble' not in (user.last_name or '').lower():
                update.message.reply_text("❌ First Name Must Have @LuckGamble")
                return
